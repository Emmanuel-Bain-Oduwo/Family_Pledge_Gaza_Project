...
class User(Base, TimestampMixin, SoftDeleteMixin):
    ...
    role: Mapped[UserRole] = mapped_column(Enum(UserRole, name="user_role"), nullable=False, default=UserRole.donor)
    ...
    notification_quran: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    notification_hadith: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    pledges: Mapped[List["Pledge"]] = relationship("Pledge", back_populates="user", foreign_keys="Pledge.user_id")
    contributions: Mapped[List["Contribution"]] = relationship("Contribution", back_populates="user", foreign_keys="Contribution.user_id")
    confirmed_contributions: Mapped[List["Contribution"]] = relationship("Contribution", back_populates="confirmed_by_user", foreign_keys="Contribution.confirmed_by")
    collector_profile: Mapped[Optional["Collector"]] = relationship("Collector", back_populates="user", uselist=False)
    collector_memberships: Mapped[List["CollectorMember"]] = relationship("CollectorMember", back_populates="donor_user")
    badges: Mapped[List["UserBadge"]] = relationship("UserBadge", back_populates="user")
    ai_drafts: Mapped[List["AiDraft"]] = relationship("AiDraft", back_populates="admin", foreign_keys="AiDraft.admin_id")
    approved_ai_drafts: Mapped[List["AiDraft"]] = relationship("AiDraft", back_populates="approved_by_user", foreign_keys="AiDraft.approved_by")
    audit_logs: Mapped[List["AdminAuditLog"]] = relationship("AdminAuditLog", back_populates="admin")
    sent_notifications: Mapped[List["Notification"]] = relationship("Notification", back_populates="sent_by_user")
    created_reminders: Mapped[List["DailyReminder"]] = relationship("DailyReminder", back_populates="created_by_user", foreign_keys="DailyReminder.created_by")
    approved_reminders: Mapped[List["DailyReminder"]] = relationship("DailyReminder", back_populates="approved_by_user", foreign_keys="DailyReminder.approved_by")
    created_campaigns: Mapped[List["Campaign"]] = relationship("Campaign", back_populates="created_by_user")
    created_projects: Mapped[List["Project"]] = relationship("Project", back_populates="created_by_user")
    created_namlef_content: Mapped[List["NamlefContent"]] = relationship("NamlefContent", back_populates="created_by_user")
    created_impact_cards: Mapped[List["ImpactCard"]] = relationship("ImpactCard", back_populates="created_by_user")
