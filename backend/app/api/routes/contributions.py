...
def my_contributions(
    ...
    items, total = contribution_service.get_my_contributions(
        db, current_user.id, skip, limit
    )
    return make_page(
        [ContributionOut.model_validate(i) for i in items], total, page, size
    )
...
@router.get(
    "/admin/contributions", response_model=PaginatedResponse[ContributionAdminOut]
)
def admin_list_contributions(
    ...
    for c in items:
        ...
        row.proof_available = bool(
            not_expired and (c.proof_object_key or c.proof_image_url)
        )
...
def admin_contribution_proof_url(
    ...
    if contribution.proof_object_key:
        return {
            "url": create_view_url(contribution.proof_object_key),
            "expires_in": settings.PROOF_SIGNED_GET_TTL_SECONDS,
            "legacy": False,
        }

    # Transitional support only for proof URLs created before private storage was
    # introduced. Deployment includes a migration/purge step so these should
    # disappear from production data.
    if contribution.proof_image_url:
        return {
            "url": contribution.proof_image_url,
            "expires_in": None,
            "legacy": True,
        }
...
@router.patch(
    "/admin/contributions/{contribution_id}/confirm", response_model=ContributionOut
)
def admin_confirm(
    contribution_id: UUID,
    current_user: User = Depends(require_admin),
...
@router.patch(
    "/admin/contributions/{contribution_id}/reject", response_model=ContributionOut
)
def admin_reject(
    contribution_id: UUID,
    ...
    return contribution_service.reject(
        db, current_user, contribution_id, data.admin_note
    )


@router.patch(
    "/admin/contributions/{contribution_id}/needs-follow-up",
...
def admin_needs_follow_up(
    ...
    return contribution_service.needs_follow_up(
        db, current_user, contribution_id, data.admin_note
    )


@router.patch(
    "/admin/contributions/{contribution_id}/review", response_model=ContributionOut
)
def admin_review_contribution(
    ...
    if status == ContributionStatus.rejected.value:
        return contribution_service.reject(
            db, current_user, contribution_id, admin_note
        )
