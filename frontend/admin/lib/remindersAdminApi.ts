...
function fail(e:unknown):never{if(axios.isAxiosError(e)&&(e.response?.status===401||e.response?.status===403))removeToken();throw new Error(getApiErrorMessage(e,'Could not complete this reminder request.'));}

export type AdminReminder = Omit<Reminder,'type'> & { type:string; reminder_type?: string; dhikr_category?: string|null; scheduled_for?: string|null; updated_at?: string; };
export type ReminderWrite = Partial<Omit<Reminder,'type'>> & { type?:string; dhikr_category?: string|null; scheduled_for?: string|null; };
