...
export interface AiTaskRun { id:string;task_id:string;status:string;planned_action?:Record<string,unknown>|null;generated_output?:{text?:string;context_used?:string[];actions_executed?:string[];[key:string]:unknown}|null;validation_result?:Record<string,unknown>|null;error_message?:string|null;executed_at?:string|null;created_at:string; }
export async function listAiTasks():Promise<AiTask[]>{try{return(await aiClient.get<AiTask[]>('/admin/ai/tasks')).data;}catch(error){return fail(error);}}
export async function createAiTask(payload:{title:string;instruction:string;task_type?:string;schedule_type?:AiScheduleType;timezone?:string;next_run_at?:string|null;requires_approval?:boolean;status?:string;}):Promise<AiTask>{try{return(await aiClient.post<AiTask>('/admin/ai/tasks',{task_type:'custom_admin_task',timezone:'Africa/Nairobi',requires_approval:true,status:'active',...payload})).data;}catch(error){return fail(error);}}
