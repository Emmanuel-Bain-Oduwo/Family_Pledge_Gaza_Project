...
export default function OperationsPage() {
  ...
  const validateCommunication=()=>{if(!form.title.trim()||!form.body.trim()||!form.channels.length){toast.error('Add a title, message and at least one delivery channel.');return false;}return true;};
  const resetComposer=()=>setForm((v)=>({...v,title:'',body:'',scheduled_for:''}));
  const sendNow=async()=>{if(!validateCommunication())return;setBusy('send-now');try{const queued=await queueCommunication({title:form.title.trim(),body:form.body.trim(),segment:form.segment,channels:form.channels,content_category:form.content_category,scheduled_for:null});const started=await runCommunication(queued.id);setCampaigns((v)=>[started,...v.filter((x)=>x.id!==started.id)]);toast.success('Sending started now. The worker will continue any remaining recipients in safe batches.');resetComposer();await load();}catch(e){toast.error(e instanceof Error?e.message:'Could not send this communication now.');}finally{setBusy(null);}};
  const scheduleLater=async()=>{if(!validateCommunication())return;if(!form.scheduled_for){toast.error('Choose the date and time first.');return;}setBusy('schedule');try{const queued=await queueCommunication({title:form.title.trim(),body:form.body.trim(),segment:form.segment,channels:form.channels,content_category:form.content_category,scheduled_for:new Date(form.scheduled_for).toISOString()});setCampaigns((v)=>[queued,...v]);toast.success('Communication scheduled.');resetComposer();await load();}catch(e){toast.error(e instanceof Error?e.message:'Could not schedule this communication.');}finally{setBusy(null);}};
  ...
  const runNow=async(c:OutboundCampaign)=>{setBusy(c.id);try{const updated=await runCommunication(c.id);setCampaigns((v)=>v.map((x)=>x.id===updated.id?updated:x));toast.success('Next delivery batch processed.');await load();}catch(e){toast.error(e instanceof Error?e.message:'Could not process communication');}finally{setBusy(null);}};
  ...
  return <AdminLayout title="Operations Command Center" subtitle="Manage donors, follow-ups, communications and today’s Family Pledge workload from one place.">
    {loading&&!stats?<div className="card p-10 text-center text-gray-400">Loading live operations…</div>:<>
      ...
      <section className="card p-5 mt-6">
        <div className="mb-5"><h2 className="font-black text-gray-900">Send a reminder or update</h2><p className="text-sm text-gray-500">Write once, choose who should receive it and where. App, Email and WhatsApp consent is checked before delivery.</p></div>
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-3"><div><label className="label">Audience</label><select className="input" value={form.segment} onChange={e=>setForm({...form,segment:e.target.value as DonorSegment})}>{SEGMENTS.map(s=><option value={s.key} key={s.key}>{s.label}</option>)}</select></div><div><label className="label">Category</label><select className="input" value={form.content_category} onChange={e=>setForm({...form,content_category:e.target.value})}>{CONTENT_CATEGORIES.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div><div className="grid grid-cols-3 gap-2">{(['app','email','whatsapp'] as CommunicationChannel[]).map(channel=><button type="button" onClick={()=>toggleChannel(channel)} key={channel} className={`rounded-xl border px-3 py-3 text-sm font-bold capitalize ${form.channels.includes(channel)?'border-primary bg-primary/10 text-primary-dark':'border-gray-200 text-gray-500'}`}>{channel==='app'?<BellRing size={16} className="mx-auto mb-1"/>:channel==='email'?<Mail size={16} className="mx-auto mb-1"/>:<MessageCircle size={16} className="mx-auto mb-1"/>}{channel}</button>)}</div><div><label className="label">Schedule for later (optional)</label><input type="datetime-local" className="input" value={form.scheduled_for} onChange={e=>setForm({...form,scheduled_for:e.target.value})}/><p className="mt-1 text-[11px] text-gray-400">Leave blank when you want to send it now.</p></div></div>
          <div className="space-y-3"><div><label className="label">Title</label><input className="input" value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="Gentle monthly Family Pledge reminder"/></div><div><label className="label">Message</label><textarea rows={6} className="input" value={form.body} onChange={e=>setForm({...form,body:e.target.value})} placeholder="Write or paste the admin-approved message here…"/></div>{preview&&<div className="grid grid-cols-4 gap-2 rounded-xl bg-gray-50 p-3 text-center"><Mini label="Audience" value={preview.total_users}/><Mini label="App" value={preview.app_eligible}/><Mini label="Email" value={preview.email_eligible}/><Mini label="WhatsApp" value={preview.whatsapp_eligible}/></div>}<div className="flex flex-wrap gap-2"><button onClick={()=>void sendNow()} disabled={busy==='send-now'||busy==='schedule'} className="btn-primary flex-1 min-w-[150px] inline-flex justify-center items-center gap-2"><Send size={15}/>{busy==='send-now'?'Starting…':'Send now'}</button><button onClick={()=>void scheduleLater()} disabled={!form.scheduled_for||busy==='send-now'||busy==='schedule'} className="btn-secondary flex-1 min-w-[150px] inline-flex justify-center items-center gap-2 disabled:opacity-50"><CalendarDays size={15}/>{busy==='schedule'?'Scheduling…':'Schedule for later'}</button><Link href="/ai/chat" className="btn-secondary inline-flex items-center gap-2"><Sparkles size={15}/>Draft with AI</Link></div></div>
        </div>
      </section>

      <section className="card p-5 mt-6"><div className="flex items-center gap-2 mb-4"><CalendarDays size={18} className="text-primary"/><h2 className="font-black text-gray-900">Communication history</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead><tr className="border-b text-left text-xs uppercase text-gray-400"><th className="py-2">Message</th><th>Segment</th><th>Channels</th><th>Status</th><th>Delivery</th><th></th></tr></thead><tbody>{campaigns.slice(0,30).map(c=><tr key={c.id} className="border-b"><td className="py-3"><div className="font-semibold">{c.title}</div><div className="text-xs text-gray-400">{new Date(c.created_at).toLocaleString()}</div></td><td>{c.segment.replace(/_/g,' ')}</td><td>{c.channels.join(', ')}</td><td className="capitalize">{c.status}</td><td>{c.sent_count}/{c.recipient_count}{c.failed_count?` · ${c.failed_count} failed`:''}</td><td><button onClick={()=>void runNow(c)} disabled={busy===c.id||c.status==='completed'} className="btn-ghost text-xs">{c.status==='completed'?'Completed':'Process next batch'}</button></td></tr>)}</tbody></table>{!campaigns.length&&<div className="py-8 text-center text-gray-400">No communication campaigns yet.</div>}</div></section>
    </>}
  </AdminLayout>;
}
