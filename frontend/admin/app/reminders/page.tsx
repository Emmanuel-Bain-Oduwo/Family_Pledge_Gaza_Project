...
const TYPES = ['quran', 'hadith', 'dua', 'dhikr', 'shirk', 'motivation', 'friday', 'sadaqah'];
...
export default function RemindersPage() {
  ...
  const handlePublish = async (item: AdminReminder) => {
    try {
      ...
      toast.success(`${typeLabel(item.type)} published and eligible opted-in users notified.`);
      await load();
    } catch (e) {
  ...
  const columns: Column<AdminReminder>[] = [
    ...
    { key: 'scheduled_for', header: 'Automatic publish', render: (r) => r.scheduled_for ? <div className="inline-flex items-center gap-1 text-xs"><CalendarClock size={13}/>{formatDateTime(r.scheduled_for)}</div> : 'Manual' },
  ...
  return <AdminLayout title="Faith, Dhikr & Motivation Content" subtitle="Draft, source-review, approve, schedule and publish donor reminders from one workspace.">
    <div className="mb-4 rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-sm leading-6 text-emerald-900"><b>One workflow for every user reminder:</b> Draft → human source/content review → Approved → scheduled automatic publish or Publish Now. Quran, Hadith, Du'a, Dhikr, Shirk Awareness, motivation, Jumu'ah and Sadaqah content are delivered only to users who opted into the relevant category. Nuanced religious rulings should be reviewed by a qualified scholar.</div>
