  ...
  return (
    <AppCard onPress={onPress} style={styles.card}>
      {impact.image_url && failedImage !== impact.image_url ? (
        ...
        <View style={[styles.imagePlaceholder, { backgroundColor: cat.color + '20' }]}><Ionicons name={cat.icon as any} size={40} color={cat.color} /></View>
      )}
      <View style={styles.content}>
        <View style={[styles.categoryBadge, { backgroundColor: cat.color + '15' }]}>
          <Ionicons name={cat.icon as any} size={12} color={cat.color} />
          <Text style={[styles.category, { color: cat.color }]}>{impact.category.charAt(0).toUpperCase() + impact.category.slice(1)}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 16 },
  categoryBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20, marginBottom: 8 },
