...
type ProfilePledgeState = 'none' | 'signed' | 'processing' | 'confirmed' | 'free_participant';
...
const STATUS_LABEL: Record<ProfilePledgeState, string> = {
  ...
  processing: 'M-PESA Processing',
  signed: 'Pledge Signed ✓',
  free_participant: 'Pledge Signed ✓',
  none: 'No Pledge',
};

function deriveProfilePledgeState(status: PaymentAwarePledgeStatus): ProfilePledgeState {
  ...
  if (status.current_month_status === 'confirmed') return 'confirmed';
  if (['created', 'initiating', 'pending'].includes(String(status.current_month_payment_status || ''))) return 'processing';
  return 'signed';
}
...
export default function ProfileScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setUser(me);
      setPledgeState(deriveProfilePledgeState(pledgeStatus as PaymentAwarePledgeStatus));
      await saveUser(me);
    } catch {
  ...
  const rateApp = async () => {
    ...
    if (!storeUrl) {
      Alert.alert('Store listing coming soon', 'The Family Pledge store listing will be linked here as soon as the first release is published.');
      return;
    }
  ...
  const performLogout = async () => {
    ...
    try {
      ...
      // Notification cleanup is best effort; local logout always wins.
    } finally {
      await logout();
  ...
  const handleLogout = () => {
    if (signingOut) return;
    if (Platform.OS === 'web') {
      const confirmed = typeof window === 'undefined' ? true : window.confirm('Sign out of Family Pledge?');
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <AppCard style={styles.card}>
        <View style={styles.cardRow}>
          <View style={styles.cardLabel}><Ionicons name="heart" size={18} color={Colors.primary} /><Text style={styles.cardLabelText}>Pledge Status</Text></View>
          <View style={[styles.statusBadge, { backgroundColor: STATUS_COLOR[pledgeState] + '20' }]}>
            <Text style={[styles.statusText, { color: STATUS_COLOR[pledgeState] }]}>{STATUS_LABEL[pledgeState]}</Text>
          </View>
        </View>
        <View style={styles.quickActions}>
          <QuickAction icon="leaf-outline" label="Journey" onPress={() => router.push('/screens/impact-journey')} />
          <QuickAction icon="flag-outline" label="Goals" onPress={() => router.push('/screens/goals')} />
          <QuickAction icon="people-outline" label="Community" onPress={() => router.push('/screens/community')} />
        </View>
      ...
      <AppCard style={styles.card}>
        ...
        <View style={styles.toggleRow}>
          ...
          <Switch value={user.anonymous_publicly} onValueChange={toggleAnonymous} disabled={updatingAnon} trackColor={{ false: Colors.gray[300], true: Colors.primary }} thumbColor={Colors.white} />
        </View>
      </AppCard>
      ...
      <View style={styles.appInfo}>
        ...
        <Text style={styles.appInfoText}>Nairobi, Kenya</Text>
      </View>
...
function MenuItem({ icon, label, detail, onPress, color = Colors.primary, last = false }: { icon: string; label: string; detail?: string; onPress: () => void; color?: string; last?: boolean }) {
...
function QuickAction({ icon, label, onPress }: { icon: string; label: string; onPress: () => void }) {
  return <TouchableOpacity style={styles.quick} onPress={onPress}><Ionicons name={icon as any} size={20} color={Colors.primary} /><Text style={styles.quickText}>{label}</Text></TouchableOpacity>;
}

function Social({ icon, onPress }: { icon: string; onPress: () => void }) {
  return <TouchableOpacity onPress={onPress} style={styles.social}><Ionicons name={icon as any} size={24} color={Colors.primaryDark} /></TouchableOpacity>;
}

const styles = StyleSheet.create({
  scroll: { flex: 1, backgroundColor: Colors.cream },
  content: { paddingBottom: 32 },
  profileHeader: { backgroundColor: Colors.primary, alignItems: 'center', paddingTop: 28, paddingBottom: 30, paddingHorizontal: 24, borderBottomLeftRadius: 28, borderBottomRightRadius: 28, marginBottom: 16 },
  avatarCircle: { width: 76, height: 76, borderRadius: 38, backgroundColor: Colors.gold, alignItems: 'center', justifyContent: 'center', marginBottom: 10, borderWidth: 3, borderColor: 'rgba(255,255,255,.4)' },
  avatarText: { fontSize: 30, fontWeight: '800', color: Colors.primaryDark },
  displayName: { fontSize: 22, fontWeight: '800', color: Colors.white },
  fullName: { fontSize: 13, color: 'rgba(255,255,255,.75)', marginTop: 2 },
  locationRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 7 },
  locationText: { color: Colors.gold, fontSize: 12, fontWeight: '500' },
  card: { marginHorizontal: 16, marginBottom: 12 },
  cardRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  cardLabel: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  cardLabelText: { fontSize: 14, fontWeight: '700', color: Colors.text.primary },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 18 },
  statusText: { fontSize: 11, fontWeight: '800' },
  quickActions: { flexDirection: 'row', gap: 8, marginTop: 14 },
  quick: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 5, backgroundColor: Colors.gray[50], borderRadius: 13, paddingVertical: 10 },
  quickText: { fontSize: 10, fontWeight: '800', color: Colors.text.primary },
  sectionTitle: { fontSize: 15, fontWeight: '900', color: Colors.text.primary, marginBottom: 6 },
  ...
  menuItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, gap: 11 },
  menuBorder: { borderBottomWidth: 1, borderBottomColor: Colors.border.light },
  menuIcon: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  menuLabel: { fontSize: 14, fontWeight: '600' },
  menuDetail: { fontSize: 10, color: Colors.text.muted, marginTop: 2 },
  socialRow: { flexDirection: 'row', justifyContent: 'center', gap: 12, marginBottom: 14 },
  social: { width: 44, height: 44, borderRadius: 22, backgroundColor: Colors.white, borderWidth: 1, borderColor: Colors.border.light, alignItems: 'center', justifyContent: 'center' },
  appInfo: { alignItems: 'center', marginVertical: 14, gap: 3 },
  appInfoText: { fontSize: 11, color: Colors.text.muted },
