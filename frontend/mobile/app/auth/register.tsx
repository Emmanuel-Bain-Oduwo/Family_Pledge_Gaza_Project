...
export default function RegisterScreen() {
  ...
  const passwordHasMinimum = form.password.length >= 8;
  ...
  const handleRegister = async () => {
    ...
    if (form.password.length < 8) { Alert.alert('Password Too Short', 'Password must have a minimum of 8 characters.'); return; }
  ...
  return <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.headerBlock}><FamilyPledgeLogo /><Text style={styles.heading}>Create Your Family Pledge Account</Text><Text style={styles.subheading}>Create your account first, then choose and sign the pledge that works for you.</Text></View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.passwordWrap}>
          <Field label="Password *" value={form.password} onChangeText={(v) => update('password', v)} placeholder="Minimum 8 characters" secureTextEntry={!showPassword} autoCapitalize="none" />
          <TouchableOpacity accessibilityRole="button" accessibilityLabel={showPassword ? 'Hide password' : 'Show password'} onPress={() => setShowPassword(!showPassword)} style={styles.eyeBtn}><Ionicons name={showPassword ? 'eye-off-outline' : 'eye-outline'} size={20} color={Colors.gray[400]} /></TouchableOpacity>
        </View>
        <Text accessibilityLiveRegion="polite" style={[styles.passwordRule, passwordHasMinimum ? styles.passwordRuleOk : styles.passwordRuleError]}>
          {passwordHasMinimum ? '✓ Password has at least 8 characters.' : 'Password must have a minimum of 8 characters.'}
        </Text>
        <Field label="Confirm Password *" value={form.confirm_password} onChangeText={(v) => update('confirm_password', v)} placeholder="Repeat password" secureTextEntry={!showPassword} autoCapitalize="none" />
        {confirmMismatch && <Text accessibilityLiveRegion="polite" style={[styles.passwordRule, styles.passwordRuleError]}>Passwords do not match.</Text>}
      </View>
      ...
      <AppButton title="Create Account" onPress={handleRegister} loading={loading} style={styles.registerBtn} />
      <TouchableOpacity onPress={() => router.push('/auth/login')} style={styles.loginLink}><Text style={styles.loginText}>Already have an account? <Text style={styles.loginBold}>Sign In</Text></Text></TouchableOpacity>
    </ScrollView>
...
const styles = StyleSheet.create({flex:{flex:1},scroll:{flex:1,backgroundColor:Colors.cream},content:{padding:20,paddingBottom:48},headerBlock:{alignItems:'center',marginBottom:24,marginTop:8},heading:{fontSize:24,fontWeight:'800',color:Colors.text.primary,marginBottom:4,textAlign:'center'},subheading:{fontSize:14,color:Colors.text.secondary,textAlign:'center'},section:{backgroundColor:Colors.white,borderRadius:16,padding:16,marginBottom:16,gap:14,shadowColor:'#000',shadowOffset:{width:0,height:1},shadowOpacity:.05,shadowRadius:4,elevation:2},sectionTitle:{fontSize:13,fontWeight:'700',color:Colors.primary,textTransform:'uppercase',letterSpacing:.8,marginBottom:2},helper:{fontSize:12,lineHeight:18,color:Colors.text.secondary},field:{gap:5},label:{fontSize:13,fontWeight:'600',color:Colors.text.primary},input:{backgroundColor:Colors.gray[50],borderRadius:10,borderWidth:1.5,borderColor:Colors.border.light,paddingHorizontal:14,paddingVertical:12,fontSize:15,color:Colors.text.primary},passwordWrap:{position:'relative'},eyeBtn:{position:'absolute',right:14,bottom:14},passwordRule:{fontSize:12,lineHeight:17,fontWeight:'700',marginTop:-8},passwordRuleError:{color:Colors.emergency},passwordRuleOk:{color:Colors.success},toggleRow:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:12},channelIcon:{width:36,height:36,borderRadius:10,backgroundColor:Colors.primary+'12',alignItems:'center',justifyContent:'center'},toggleInfo:{flex:1},toggleLabel:{fontSize:14,fontWeight:'600',color:Colors.text.primary,marginBottom:2},toggleDesc:{fontSize:12,color:Colors.text.secondary,lineHeight:17},registerBtn:{marginBottom:16},loginLink:{alignItems:'center',padding:8},loginText:{fontSize:14,color:Colors.text.secondary},loginBold:{color:Colors.primary,fontWeight:'700'}});