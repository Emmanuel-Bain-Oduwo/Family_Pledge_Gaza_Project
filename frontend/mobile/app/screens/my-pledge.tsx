...
export default function MyPledgeScreen() {
  ...
  return <ScrollView style={styles.scroll} contentContainerStyle={styles.content}><View style={styles.hero}><Ionicons name="heart" size={32} color={Colors.goldLight} /><Text style={styles.title}>My Family Pledge</Text><Text style={styles.subtitle}>{data?.has_active_pledge ? 'Your pledge is active.' : 'Choose a monthly pledge or join as a free participant.'}</Text></View>{pledge ? <AppCard style={styles.card}><Row label="Status" value={pledge.status} /><Row label="Pledge" value={pledge.pledge_type === 'free_participant' ? 'Free participant' : `${pledge.currency} ${Number(pledge.amount).toLocaleString()} monthly`} /><Row label="Started" value={pledge.start_date ? new Date(pledge.start_date).toLocaleDateString() : '—'} /><Row label="Confirmed contributions" value={data?.confirmed_contributions_count ?? 0} /><Row label="This month" value={data?.current_month_contributed ? 'Confirmed' : 'Not confirmed yet'} last /></AppCard> : <AppCard style={styles.card}><Text style={styles.empty}>You have not signed a pledge yet.</Text></AppCard>}<AppButton title={pledge ? 'Contribute / Update My Journey' : 'Sign My Pledge'} onPress={() => router.push('/screens/contribute')} /></ScrollView>;
}
